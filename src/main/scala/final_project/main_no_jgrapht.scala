 ...
 * main.scala
 *
 * A driver that supports the (1+ε)-approximate semi-streaming matching of Fischer et al.
 * JGraphT dependency has been removed for Java 8 compatibility
 */
object main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val mode = "approx"  // Only support approx mode
    val inPath = args(1)
    val outPath = args(2)
    ...
    println(s"Starting approximate matching algorithm on ${inPath} with epsilon=${epsilon}")
    val resultGraph = runApproxMatching(baseGraph, epsilon)
  ...
  def runApproxMatching(
    ...
    val finalGraph = (1 to numPhases).foldLeft(greedy) { (g, phase) =>
      ...
      // Continue with the consistent graph
      g5
    }
